 ...
 * 动画状态机类
 */
type StateCallback = (() => void) | null
type TransitionCallback = ((from: string, to: string) => void) | null
...
export class AnimationStateMachine {
  ...
  _callbacks: {
    ...
    onEnter: Record<string, StateCallback>
    onExit: Record<string, StateCallback>
  }

  constructor(initialState: string = AnimationState.IDLE) {
   ...
   * 注册状态进入回调
   */
  onEnter(state: string, callback: () => void) {
   ...
   * 注册状态退出回调
   */
  onExit(state: string, callback: () => void) {
